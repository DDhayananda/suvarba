...
#
#********START USER_INPUTS*********
TESTSIGMA_API_KEY=eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiI2NTE0MWM4Zi1mNjg0LTRjOTgtYjRhYS05ODA2MTEyOTU2N2YiLCJkb21haW4iOiJ0ZXN0c2lnbWF0ZWNoLmNvbSIsInRlbmFudElkIjoyODE3LCJpc0lkbGVUaW1lb3V0Q29uZmlndXJlZCI6ZmFsc2V9.QL5dXxeuDBqs0SF3sEP-cCTWs67_Rw5diqfwplP1V-8UNku8vK72n9U42qKW7X7aCjMNo0AfkOCM_HcxIGFsJQ
MAX_WAIT_TIME_FOR_SCRIPT_TO_EXIT=60
JUNIT_REPORT_FILE_PATH=./junit-report-$(date +"%Y%m%d%H%M").xml
BUILD_NO=$(date +"%Y%m%d%H%M")
TESTSIGMA_TEST_PLAN_ID=1713
#********END USER_INPUTS***********
 
#********GLOBAL variables**********
POLL_COUNT=$((MAX_WAIT_TIME_FOR_SCRIPT_TO_EXIT*2))
...
function checkTestPlanRunStatus(){
  ...
  do
    ...
    if [ $EXECUTION_STATUS = "STATUS_IN_PROGRESS" ] || [ $EXECUTION_STATUS = "STATUS_CREATED" ]; then
      ...
      echo "Unexpected Execution status $EXECUTION_STATUS. Please check run results for more details."
    fi
  done
}
